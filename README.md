...
Nix stages its own dependency directory; the repository's `vendor/` and Cargo
source redirection remain available to native builds. It installs the public
headers and `libkrun.pc` alongside the library.
...
Native Make builds also honor `CARGO_TARGET_DIR`; `CARGO_FLAGS` can pass
`--locked --offline` when using the vendored dependency tree.
